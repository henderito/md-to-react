...
### Changed
- **CI/CD Pipeline**: Replaced custom script-based version bumping with Google's `release-please-action` to automatically generate pull requests for releases, respecting branch protection rules.
- **Git Hooks**: Replaced custom `prepare-commit-msg` prompt with `commitlint` via a `commit-msg` Husky hook to strictly enforce Conventional Commits locally.
